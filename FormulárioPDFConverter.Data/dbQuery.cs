...
namespace FormulárioPDFConverter.Data
{
    public class dbQuery
    {
        ...
        public bool VerificarEmailJaEnviado(string idEmpresa)
        {
            ...
            {
                string query = "SELECT EmailEnviado FROM Empresa WHERE ID_Empresa = @ID_Empresa";
        ...
        public void UpdateEmailEnviado(string idEmpresa)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                string query = "UPDATE Empresa SET EmailEnviado = 1 WHERE ID_Empresa = @ID_Empresa";

                connection.Execute(query, new { ID_Empresa = idEmpresa });
            }
        }

        public CadastroData GetCadastroById(string ID_Empresa)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                string sql = @"SELECT * FROM cadastro Where ID_Empresa = @ID_Empresa";
                return connection.QueryFirstOrDefault<CadastroData>(sql, new { ID_Empresa = ID_Empresa });
            }
        }

        public List<UploadFilesData> GetDocumentosById(string ID_Empresa)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                string sql = @"SELECT * FROM UploadFiles WHERE ID_Empresa = @ID_Empresa";
                return connection.Query<UploadFilesData>(sql, new { ID_Empresa = ID_Empresa }).ToList();
            }
        }
    }
}
